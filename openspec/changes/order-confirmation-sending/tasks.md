...
- [ ] **B1: OrderConfirmation model** — Create model with fields: tenant, contract (FK), created_by, created_at, sent_at, sent_to (JSON), email_message_id, additional_emails (JSON), personal_message, pdf_file, language, status. Add migration.
...
- [ ] **B4: AB email sending task** — New Celery task `send_order_confirmation_email` using existing M365 Graph API integration. Attach PDF. Send to billing_emails + additional_emails.
...
- [ ] **F1: Activation dialog AB step** — After checklist validation, show AB prompt with preview panel, personal message textarea, additional emails input, "Send & Activate" / "Skip & Activate" buttons.
